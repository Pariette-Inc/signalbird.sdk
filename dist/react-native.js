...
// src/react-native/index.ts
function asyncStorageAdapter(store) {
  return {
...
function createSignalbirdApp(config) {
  return new SignalbirdApp(config);
}
function useNativeChat(client, options = {}) {
  const foregroundRef = react.useRef(options.isForeground ?? true);
  ...
  const [state, setState] = react.useState(session.snapshot());
  react.useEffect(() => {
...
exports.SignalbirdApp = SignalbirdApp;
exports.asyncStorageAdapter = asyncStorageAdapter;
exports.createSignalbirdApp = createSignalbirdApp;
exports.useNativeChat = useNativeChat;
exports.useNativePush = useNativePush;
